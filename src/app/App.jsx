...
const t = (lang, key, vars={}) => {
  const dict = {
    en: {
      ...
      base: 'Base', decimals: 'Decimals', loadSample: 'Load Sample', export: 'Export JSON', import: 'Import JSON',
      ...
      expenses: 'Expenses',
      eqSplit: 'equal split', uneqSplit: 'unequal (weights)',
      addExpense: '+ Add expense',
      share: 'share', among: 'among',
      debts: 'Direct Debts (A owes B)', addDebt: '+ Add debt',
      results: 'Results', copySettlement: 'Copy settlement',
      netsTitle: (bc) => `Net balances (in ${bc}) — + receive / – pay`,
      settleTitle: (bc) => `Minimal transactions (in ${bc})`,
      allSettled: 'All settled 🎉',
      owes: 'owes', to: 'to', payWith: 'Pay with',
      nequi: 'Nequi', daviplata: 'DaviPlata', whatsapp: 'WhatsApp', qr: 'QR',
    ...
    es: {
      ...
      base: 'Base', decimals: 'Decimales', loadSample: 'Cargar ejemplo', export: 'Exportar JSON', import: 'Importar JSON',
...
// Convert an amount from currency to base using fx map where fx[code] = base per 1 code
...
// Try to open Android app by package (best-effort); fallback to store
...
export default function App() {
  ...
  const [ledgers, setLedgers] = useState(() => {
    ...
    const id = uid()
    return [{
      id, name: 'Viaje 1', baseCurrency: 'COP', decimals: 1,
      fx: { COP: 1, USD: 4000, EUR: 4300 },
      people: [
        { id: uid(), name: 'Santi', phone: '' },
        { id: uid(), name: 'Alejandro', phone: '' },
        { id: uid(), name: 'Alfonso', phone: '' },
        { id: uid(), name: 'Miguel', phone: '' },
        { id: uid(), name: 'Fer', phone: '' },
        { id: uid(), name: 'Juan', phone: '' },
        { id: uid(), name: 'Samuel', phone: '' },
      ],
  ...
  // ---- Derived ----
  const balances = useMemo(() => computeBalances(ledger.people, ledger.expenses, ledger.directDebts, ledger.fx), [ledger])
  const settlements = useMemo(() => simplifyBalances(balances, ledger.decimals), [balances, ledger.decimals])
  ...
  const addLedger = () => {
    ...
    setLedgers((arr) => [...arr, {
      ...
      fx: { ...ledger.fx }, people: [], expenses: [], directDebts: []
    }])
    setLid(id)
  }
  ...
  const deleteLedger = () => {
    ...
    setLedgers(arr)
    setLid(arr[Math.max(0, idx-1)]?.id || null)
  }
  const renameLedger = () => {
  ...
  // ---- People ----
  const addPerson = () => updateLedger({ people: [...ledger.people, { id: uid(), name: `Person ${ledger.people.length + 1}`, phone: '' }] })
  ...
  const addCurrency = () => {
    ...
    updateLedger({ fx: { ...ledger.fx, [code]: 1 } })
  }
  ...
  // ---- Sample ----
  const loadSample = () => {
    const ids = ledger.people.slice(0,7).map(p => p.id)
    const [Santi, Ale, Alfo, Mig, Fer, Juan, Sam] = ids
    updateLedger({
      expenses: [
        { id: uid(), label: 'Dinner', amount: 210000, currency: 'COP', payerId: Santi, participants: [Santi,Fer,Alfo,Ale,Mig], splitMode:'equal', weights: {} },
        { id: uid(), label: 'House services', amount: 90000, currency: 'COP', payerId: Ale, participants: [Juan,Sam,Fer,Mig,Santi,Alfo], splitMode:'equal', weights: {} },
        { id: uid(), label: 'Taxi', amount: 25, currency: 'USD', payerId: Mig, participants: [Mig,Santi,Ale,Fer,Alfo], splitMode:'weights', weights: { [Mig]:2, [Santi]:1, [Ale]:1, [Fer]:1, [Alfo]:1 } },
      ],
      directDebts: [
        { id: uid(), fromId: Sam, toId: Alfo, amount: 15000, currency: 'COP' },
        { id: uid(), fromId: Mig, toId: Alfo, amount: 15000, currency: 'COP' },
        { id: uid(), fromId: Mig, toId: Juan, amount: 27300, currency: 'COP' },
        { id: uid(), fromId: Alfo, toId: Juan, amount: 13300, currency: 'COP' },
        { id: uid(), fromId: Alfo, toId: Mig, amount: 30000, currency: 'COP' },
      ]
    })
  }
  ...
  const importJSON = (file) => {
    ...
    reader.onload = () => {
      try { setLedgers(JSON.parse(String(reader.result))); } catch { alert('Invalid JSON') }
    }
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-900 dark:text-slate-100 p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        {/* Top bar: language, theme & ledger */}
        <header className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold">{t(lang,'title')}</h1>
            <p className="text-sm text-slate-600 dark:text-slate-400">{t(lang,'subtitle')}</p>
          </div>
          <div className="flex flex-wrap gap-3 items-center">
            ...
            <button className="px-3 py-1.5 rounded bg-slate-800 dark:bg-slate-700 text-white text-sm" onClick={addLedger}>{t(lang,'addLedger')}</button>
            <button className="px-3 py-1.5 rounded bg-slate-800 dark:bg-slate-700 text-white text-sm" onClick={duplicateLedger}>{t(lang,'duplicate')}</button>
            <button className="px-3 py-1.5 rounded bg-slate-800 dark:bg-slate-700 text-white text-sm" onClick={renameLedger}>{t(lang,'rename')}</button>
            <button className="px-3 py-1.5 rounded bg-rose-600 text-white text-sm" onClick={deleteLedger}>{t(lang,'delete')}</button>
            <button className="px-3 py-1.5 rounded bg-slate-900 dark:bg-black text-white text-sm" onClick={exportJSON}>{t(lang,'export')}</button>
          ...
          <div className="mt-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex gap-2">
                <button className="px-3 py-1.5 rounded bg-slate-800 dark:bg-slate-700 text-white text-sm" onClick={addCurrency}>{t(lang,'addCurrency')}</button>
              </div>
            </div>
            <div className="grid md:grid-cols-3 gap-3">
              {Object.entries(ledger.fx).map(([code, rate]) => (
                <div key={code} className="border border-slate-300 dark:border-slate-700 rounded-xl p-3 flex items-center gap-2 bg-white dark:bg-slate-900">
                  ...
                  <button className="text-sm px-2 py-1 rounded bg-rose-600 text-white" onClick={() => removeCurrency(code)} disabled={code === ledger.baseCurrency} title={code === ledger.baseCurrency ? t(lang,'cannotRemoveBase') : t(lang,'remove')}>×</button>
                </div>
              ))}
            </div>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-2">{t(lang,'fxHint')}</p>
          </div>
        </section>
        ...
        <section className="bg-white dark:bg-slate-800 rounded-2xl shadow p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <button className="px-3 py-1.5 rounded bg-slate-800 dark:bg-slate-700 text-white text-sm" onClick={addPerson}>{t(lang,'addPerson')}</button>
          </div>
          <div className="grid md:grid-cols-3 gap-3">
            {ledger.people.map((p) => (
              <div key={p.id} className="border border-slate-300 dark:border-slate-700 rounded-xl p-3 space-y-2 bg-white dark:bg-slate-900">
                <div className="flex items-center gap-2">
                  <input className="flex-1 px-2 py-1 rounded border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800" value={p.name} onChange={(e) => updateLedger({ people: ledger.people.map(x => x.id===p.id? {...x, name: e.target.value } : x) })} />
                  <button className="text-sm px-2 py-1 rounded bg-rose-600 text-white" onClick={() => removePerson(p.id)} disabled={ledger.people.length <= 2}>×</button>
                </div>
                <input className="w-full px-2 py-1 rounded border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800" placeholder={t(lang,'phone')} value={p.phone||''} onChange={(e) => updateLedger({ people: ledger.people.map(x => x.id===p.id? {...x, phone: e.target.value } : x) })} />
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="bg-white dark:bg-slate-800 rounded-2xl shadow p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex gap-2">
              <button className="px-3 py-1.5 rounded bg-indigo-600 text-white text-sm shadow hover:bg-indigo-700" onClick={loadSample}>{t(lang,'loadSample')}</button>
              <button className="px-3 py-1.5 rounded bg-slate-800 dark:bg-slate-700 text-white text-sm" onClick={addExpense}>{t(lang,'addExpense')}</button>
            </div>
          </div>
                ...
                <div className="grid md:grid-cols-8 gap-2 items-center">
                  ...
                  <div className="md:col-span-2 flex flex-wrap gap-2">
                    {ledger.people.map((p) => {
                      ...
                      return (
                        <label key={p.id} className={`text-sm px-2 py-1 rounded border cursor-pointer ${on ? 'bg-emerald-50 dark:bg-emerald-900/20 border-emerald-300 dark:border-emerald-700' : 'bg-white dark:bg-slate-800 border-slate-300 dark:border-slate-700'}`}>
                  ...
                  <button className="px-3 py-1.5 rounded bg-rose-600 text-white text-sm" onClick={() => removeExpense(e.id)}>{t(lang,'remove')}</button>
                </div>
        ...
        <section className="bg-white dark:bg-slate-800 rounded-2xl shadow p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <button className="px-3 py-1.5 rounded bg-slate-800 dark:bg-slate-700 text-white text-sm" onClick={addDebt}>{t(lang,'addDebt')}</button>
          </div>
          <div className="space-y-3">
            {ledger.directDebts.map((d) => (
              <div key={d.id} className="border border-slate-300 dark:border-slate-700 rounded-xl p-3 grid md:grid-cols-7 gap-2 items-center bg-white dark:bg-slate-900">
                ...
                <button className="px-3 py-1.5 rounded bg-rose-600 text-white text-sm" onClick={() => removeDebt(d.id)}>{t(lang,'remove')}</button>
              </div>
            ))}
        ...
        <section className="bg-white dark:bg-slate-800 rounded-2xl shadow p-4 space-y-4">
            ...
            <div className="grid md:grid-cols-3 gap-3">
              {ledger.people.map((p) => (
                <div key={p.id} className="border border-slate-300 dark:border-slate-700 rounded-xl p-3 flex items-center justify-between bg-white dark:bg-slate-900">
                  <span>{p.name}</span>
                  <span className={balances[p.id] >= 0 ? 'text-emerald-500' : 'text-rose-400'}>
          ...
          <div>
            ...
            {settlements.length === 0 ? (
              <p className="text-sm text-slate-500 dark:text-slate-400">{t(lang,'allSettled')}</p>
            ) : (
